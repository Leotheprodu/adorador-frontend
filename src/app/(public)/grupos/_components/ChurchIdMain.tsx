...
import { SongProps } from '@app/(public)/grupos/[churchId]/canciones/_interfaces/songsInterface';
import { getSongsOfChurch } from '@app/(public)/grupos/[churchId]/canciones/_services/songsOfChurchService';
import { getChurchById } from '@app/(public)/grupos/_services/churchesService';
        ...
        <div className="my-4 flex flex-col justify-center">
          ...
          <ul className="flex flex-wrap justify-center gap-3">
            {LimitedListOfSongs?.map((song) => (
              <li key={song.id}>
                <Link href={`/grupos/${churchId}/canciones/${song.id}`}>
          ...
          {songs && songs?.length > limitOfSongs && (
            <div className="flex justify-center">
              <Button
                ...
                variant="ghost"
                as={Link}
                href={`/grupos/${churchId}/canciones`}
                className="mt-4 w-48"
              >
            ...
            {data?.events
              ...
              .map((event) => (
                <li className="flex flex-wrap" key={event.id}>
                  <Link href={`/grupos/${churchId}/eventos/${event.id}`}>
                    <div
