      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="flex items-center gap-3 rounded-lg bg-slate-50 p-4 dark:bg-gray-900/60">
          {event.eventMode === 'videolyrics' ? (
            <VideoCameraIcon className="h-5 w-5 text-brand-blue-600 dark:text-brand-blue-400" />
          ) : (
            <GuitarIcon className="h-5 w-5 text-brand-purple-600 dark:text-brand-purple-400" />
          )}
          <div>
            <p className="text-xs text-slate-500 dark:text-slate-300">Tipo</p>
            <p
              className={`font-semibold ${event.eventMode === 'videolyrics' ? 'text-brand-blue-700 dark:text-brand-blue-300' : 'text-brand-purple-700 dark:text-brand-purple-300'}`}
            >
              {event.eventMode === 'videolyrics'
                ? 'Video Lyrics'
                : 'Banda en Vivo'}
