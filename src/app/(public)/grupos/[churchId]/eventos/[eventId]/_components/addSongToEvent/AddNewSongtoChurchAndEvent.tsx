import {
  addSongsToChurchService,
  getSongsOfChurch,
} from '@app/(public)/grupos/[churchId]/canciones/_services/songsOfChurchService';
import {
  Button,
  Modal,
  ModalBody,
...
import { SongPropsWithoutId } from '@app/(public)/grupos/[churchId]/canciones/_interfaces/songsInterface';
...
export const AddNewSongtoChurchAndEvent = ({
  ...
  const { data: songsOfChurch, status: statusGetSongs } = getSongsOfChurch({
    churchId,
  });
  const {
    data: newSong,
  ...
  } = addSongsToChurchService({ params });
  ...
  useEffect(() => {
    if (statusAddSongToEvent === 'success') {
      ...
      refetch();
      if (goToSong) {
        router.push(`/grupos/${params.churchId}/canciones/${newSong?.id}`);
      }
      onOpenChange();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
